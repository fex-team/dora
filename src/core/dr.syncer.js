...
//            md = me.codeeditor.getPlainTxt();
//            html = editor.mdparser.md2html(md);
//            me.richeditor.setContent(html);
//        } else if (current === me.richeditor) {
//            html = me.richeditor.getContent();
//            md = editor.mdparser.html2md(html);
//            me.codeeditor.setContent('<pre>' + md + '</pre>');
//        }
//
//        me.oldMd = md;
//        me.oldHtml = me.richeditor.body.innerHTML;
    },
    syncPart: function(current){
        var me = this,
            editor = me.editor,
            diff, newMd = '', newHtml = [];
        ...
        if (current === me.codeeditor) {
            ...
            diff = me.makeMdPatch(me.oldMd, newMd);

            for (var i = 0; i < diff.change.length; i++) {
                newHtml.push(editor.mdparser.md2html(diff.change[i]));
            }

            me.setHtmlPartContent(newHtml, diff.start, diff.end);
        } else if (current === me.richeditor) {
            newHtml = me.richeditor.getContent();
            newMd = editor.mdparser.html2md(newHtml);
            me.setMdPartContent(newHtml);
        }
    ...
    makeMdPatch: function(oldMd, newMd){
        var me = this;
        var oldBlocks = me.splitMdBlocks(newMd);
        var newBlocks = me.splitMdBlocks(oldMd);
    ...
    applyMdPatch: function(oldMd, patch){
        var output = [],
            newBlocks = this.splitMdBlocks(oldMd);
